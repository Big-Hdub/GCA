        ...
        {course &&
            <div className={`flex gap-15 column acenter jcenter padding-15 h100 font-${font} ${theme}`}>
                <div className="flex gap-15">
                    <p>Subject:</p>
                    <p className={`link ${theme} no-dec`} onClick={() => linkClick(`/courses/${course.id}`)}>{course.title}</p>
                    <p>{course.level}</p>
                </div>
                <div className="course-image-container">
                    <img onClick={() => linkClick(`/courses/${course.id}`)} src={course.image} className={`course-image link ${theme} no-dec`} />
                </div>
